class UserMailer < ActionMailer::Base
  ...
  def notification_email(who, review, type)
    ...
    if type == 'like' || type == 'comment' 
      ...
      @likes_review = Review.where("reviews.user_id = ? AND reviews.count_likes != 0", review.user_id).includes(:likes).where("likes.id > ? AND likes.user_id != ?", last_like_id, review.user_id)
      @comments_review = Review.where("reviews.user_id = ? AND reviews.count_comments != 0", review.user_id).includes(:comments).where("comments.id > ? AND comments.user_id != ?", last_comment_id, review.user_id)
