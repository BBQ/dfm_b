class Review < ActiveRecord::Base
  ...
  def format_review_for_api(user_id=nil)    
    ...
    data = {
      ...
      :text => text,
      :dish_id => dish_id,
      :dish_name => review_dish ? review_dish.name : 'Name is missing :(',
      :dish_votes => review_dish ? review_dish.votes : 0,
      ...
      :dish_rating => review_dish ? review_dish.rating : 0,
  ...
  def self.save_review(user_review)
    ...
    if rating > 0
      ...
      if user_review[:photo].blank? && review = review_exist?(user_review[:user_id], user_review[:dish_id])
        ...
        if user_review[:rtype] != 'home_cooked' && user_review[:rtype] != 'delivery'
