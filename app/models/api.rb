...
class API < ActiveRecord::Base
  ...
  def self.api_get_restaurant(id, type, user_id)
    ...
    if restaurant        
      ...
      better_networks = Network.where('fsq_checkins_count >= ?', restaurant.network.fsq_checkins_count).count.to_f
      popularity = (100 * better_networks / Network.where('fsq_checkins_count > 0').count.to_f).round(0)
