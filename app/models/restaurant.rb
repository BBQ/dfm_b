...
class Restaurant < ActiveRecord::Base
  ...
  def find_image
    ...
    elsif dish = Dish.select(:photo).find_by_photo_and_network_id("IS NOT NULL", network_id).order('rating DESC, votes DESC')
      dish.photo
    elsif review = Review.select(:photo).where("network_id = ?", id).network_id('count_likes DESC').first
      review.photo
    end
  end
