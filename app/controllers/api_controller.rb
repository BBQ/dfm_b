class ApiController < ApplicationController
  ...
  def get_dishes
    ...
    dishes = dishes.select('dishes.id, dishes.name, dishes.rating, dishes.votes, dishes.photo, dishes.network_id, networks.id, networks.name AS t0_r1').joins('LEFT OUTER JOIN `networks` ON `networks`.`id` = `dishes`.`network_id`')
