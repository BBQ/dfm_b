...
class ApiController < ApplicationController
  ...
  def get_restaurants
    ...
    restaurants.each do |r|
      ...
      if dont_add == 0
        dishes = []
        r.network.dishes.select('DISTINCT dishes.id, dishes.name, dishes.photo, dishes.rating, dishes.votes, dishes.dish_type_id').order("(dishes.rating - 3)*dishes.votes DESC, dishes.photo DESC").joins(:reviews).where("dishes.photo IS NOT NULL || (dishes.rating > 0 AND reviews.photo IS NOT NULL)").limit(num_images).each do |dish|
            dishes.push({
              :id => dish.id,
              :name => dish.name,
              :photo => dish.image_sd,
