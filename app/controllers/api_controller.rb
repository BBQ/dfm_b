...
class ApiController < ApplicationController
  ...
  def get_dishes
    ...
    if top_user_id > 0
      ...
      dishes = Dish.select([:id, :name, :rating, :votes, :photo, :network_id, :fsq_checkins_count]).where("top_user_id = ?",top_user_id).order("votes DESC, photo DESC, fsq_checkins_count DESC")
      dishes.each do |d|
  ...
  def get_restaurants
    ...
    if top_user_id > 0
      ...
      ON `restaurants`.`id` = `r1`.`id`").where('restaurants.lat IS NOT NULL AND restaurants.lon IS NOT NULL AND top_user_id = ?',top_user_id).order("restaurants.fsq_checkins_count DESC, networks.rating DESC, networks.votes DESC").group('restaurants.name')
