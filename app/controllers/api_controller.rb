class ApiController < ApplicationController
  ...
  def get_dishes
    ...
    dishes = dishes.select('dishe.id, dishe.name, dishe.rating, dishe.votes, dishe.photo, network.id, network.name').joins('LEFT OUTER JOIN `restaurants` ON `restaurants`.`network_id` = `dishes`.`network_id`')
