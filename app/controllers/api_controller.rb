class ApiController < ApplicationController
  ...
  def get_dishes
    ...
    dishes = dishes.select('dishes.id, dishes.name, dishes.rating, dishes.votes, dishes.photo, networks.id, networks.name').joins('LEFT OUTER JOIN `restaurants` ON `restaurants`.`network_id` = `dishes`.`network_id`')
