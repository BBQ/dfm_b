class RestaurantsController < ApplicationController
  ...
  def show
    if @restaurant = Restaurant.find_by_id(params[:id])
      ...
      @dishes_wimg = Dish.select([:id, :photo, :name]).where("network_id = ? AND photo IS NOT NULL", @restaurant.network_id).limit(21).order("rating/votes DESC") unless @best_dishes.nil?
      @any_dishes = Dish.select([:id, :photo, :name]).where("network_id = ?", @restaurant.network_id).limit(21).order("rating/votes DESC") unless @dishes_wimg.nil?
